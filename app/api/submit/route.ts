...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[${requestId}] Received submission request for order:`, orderId, 'Order #:', orderNumber);
    ...
    console.log(`[${requestId}] Webhook URL configured:`, webhookUrl ? 'Yes' : 'No');
    console.log(`[${requestId}] Webhook URL:`, webhookUrl);

    if (!webhookUrl) {
      console.error(`[${requestId}] N8N webhook URL not configured`);
      return NextResponse.json(
    ...
    try {
      ...
      console.log(`[${requestId}] n8n webhook response status:`, response.status);
      ...
      console.log(`[${requestId}] n8n webhook response:`, responseData);

      // Return success with any data from n8n workflow
      return NextResponse.json({
        success: true,
        message: 'Prescription form submitted successfully',
        data: responseData, // Include response from n8n workflow
      });
      ...
      if (fetchError.name === 'AbortError') {
        console.error(`[${requestId}] n8n webhook request timed out after 120 seconds`);
        return NextResponse.json(
          { error: 'Webhook request timed out after 120 seconds' },
          { status: 500 }
        );
      }
    ...
    console.error(`[${requestId}] Failed to send to n8n:`, error);
